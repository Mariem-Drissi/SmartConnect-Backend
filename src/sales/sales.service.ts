...
export class SalesService {
  ...
  async createOrder(dto: CreateOrderDto, userId: string) {
    ...
    if (user?.role === "COMMERCIAL") {
      await this.stock.decrementForSale(
        userId,
        dto.items.map((i) => ({
      ...
      data: {
        ...
        items: {
          create: dto.items.map((i) => ({
            productId: i.productId,
            productName: productNames.get(i.productId) ?? "Produit supprimé",
            quantity: i.quantity,
  ...
  async deleteArchived(id: string) {
    ...
    if (!order.isArchived)
      throw new BadRequestException(
        "La commande doit être archivée avant suppression définitive",
      );
  ...
  private generateInvoiceHtml(order: any): string {
    const items = order.items
      .map(
        (i: any) =>
          `<tr><td style="padding:8px;border-bottom:1px solid #242B3D;color:#C4C9D6">${i.product.name}</td><td style="padding:8px;border-bottom:1px solid #242B3D;text-align:center;color:#C4C9D6">${i.quantity}</td><td style="padding:8px;border-bottom:1px solid #242B3D;text-align:right;color:#C4C9D6">${Number(i.unitPrice).toFixed(3)} TND</td><td style="padding:8px;border-bottom:1px solid #242B3D;text-align:right;color:#E7C27D;font-weight:bold">${Number(i.subtotal).toFixed(3)} TND</td></tr>`,
      )
      .join("");
  ...
  async getSalesStats(createdById?: string) {
    const where = createdById
      ? { createdById, status: { not: "CANCELLED" as any } }
