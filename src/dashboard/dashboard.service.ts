...
export class DashboardService {
  ...
  async getKpis(userId?: string, role?: string) {
    ...
    const orderFilter = isAdmin
      ? { isArchived: false, status: { not: "CANCELLED" as any } }
      : {
          isArchived: false,
          createdById: userId,
    ...
    const [
      ...
      this.prisma.clientScore.count({
        where: { inactivityScore: { gte: 60 } },
      }),
      this.prisma.productStock.findMany({ include: { product: true } }),
      this.prisma.commercialStock.findMany({
    ...
    const lowCommercialStockCount = lowCommercialStocks.filter(
      (s) => s.quantity <= s.minThreshold,
    ).length;
    const som = new Date(new Date().getFullYear(), new Date().getMonth(), 1);
    const ordersThisMonth = await this.prisma.saleOrder.findMany({
      where: { ...orderFilter, createdAt: { gte: som } },
    });
    const revenueThisMonth = ordersThisMonth.reduce(
      (s, o) => s + Number(o.totalAmount),
      0,
    );
    const recentOrders = await this.prisma.saleOrder.findMany({
      where: orderFilter,
      take: 5,
      orderBy: { createdAt: "desc" },
      include: { client: true },
    });
    const topClients = await this.prisma.saleOrder.groupBy({
      by: ["clientId"],
    ...
    const topClientsWithData = await Promise.all(
    ...
    return {
      kpis: {
        ...
        atRiskClients,
        lowStockProducts,
        lowCommercialStockCount,
        pendingVisits,
        prospects,
      },
      recentOrders,
      topClients: topClientsWithData,
      salesByMonth,
    };
  }
