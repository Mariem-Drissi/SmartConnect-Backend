...
export class AnalyticsService {
  ...
  async byCommercial(since?: Date) {
    ...
    const commercials = await this.prisma.user.findMany({
      where: { role: { in: ['COMMERCIAL', 'RESPONSABLE_COMMERCIAL'] }, isActive: true },
    ...
    for (const c of commercials) {
      const orders = await this.prisma.saleOrder.findMany({
        where: { createdById: c.id, status: { not: 'CANCELLED' }, ...dateFilter },
      ...
      const conversionRate = completedVisits > 0 ? Math.round((visitsWithOrder / completedVisits) * 100) : 0;

      const assignedClients = await this.prisma.client.count({ where: { assignedUserId: c.id, isActive: true } });
      ...
      results.push({
        ...
        avgOrderValue: orders.length > 0 ? Math.round((revenue / orders.length) * 1000) / 1000 : 0,
  ...
  async byProduct(since?: Date) {
    const items = await this.prisma.saleOrderItem.findMany({
      where: { order: { status: { not: 'CANCELLED' }, ...(since && { createdAt: { gte: since } }) } },
      select: { quantity: true, subtotal: true, product: { select: { id: true, name: true, sku: true, flavor: true, category: { select: { name: true, color: true } } } } },
    });
  ...
  async byCategory(since?: Date) {
    ...
    for (const p of products) {
      ...
      if (!map.has(key)) {
        map.set(key, { category: key, color: p.categoryColor, revenue: 0, quantitySold: 0, productCount: 0 });
      }
      const entry = map.get(key);
